...
function getElement(searchTerm){
	...
	for(var i=0;i<elements.length;i++){
		...
		var is原子序数=new RegExp("[0-9]+")
		if(is原子序数.test(searchTerm)){
			var num=parseInt(is原子序数.exec(searchTerm)[0])
			if(num!=searchTerm){
				摩尔=num
				searchTerm=searchTerm.replace(摩尔,"")
			}
		}
